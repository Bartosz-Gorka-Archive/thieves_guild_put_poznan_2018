...
void send(int &clock, int message, int extra_message, int tag, int receiverID, int senderID) {
  ...
  data[0] = clock;
  data[1] = message;
  data[2] = extra_message;

  // Send data with MPI action
  MPI_Send(&data, 3, MPI_INT, receiverID, tag, MPI_COMM_WORLD);

  // When enabled debug mode - show extra details
  if (debug_mode) {
    printf("[%05d][%02d][TAG: %03d] Send '%d' and '%d' to process %d.\n",
           data[0], senderID, tag, data[1], data[2], receiverID);
  }
...
void receive(int &clock, int data[], MPI_Status &status, int tag, int receiverID, int senderID) {
  // Receive data
  if(tag != -1) {
    MPI_Recv(data, 3, MPI_INT, senderID, tag, MPI_COMM_WORLD, &status);
  } else {
    MPI_Recv(data, 3, MPI_INT, senderID, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
  }
  ...
  // When enabled debug mode - show extra details
  if (debug_mode) {
    printf("[%05d][%02d][TAG: %03d] Receive '%d' and '%d' from process %d.\n",
           clock, receiverID, status.MPI_TAG, data[1], data[2], status.MPI_SOURCE);
  }
...
void broadcast(int &clock, int message, int extra_message, int tag, int world_size, int senderID) {
  ...
  for(int i = 0; i < world_size; i++) {
    ...
    if(i != senderID) {
      send(clock, message, extra_message, tag, i, senderID);
    }
  }
}
